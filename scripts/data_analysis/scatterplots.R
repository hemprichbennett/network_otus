...
df[which(df$network == "SAFE"), "network"] <- "SAFE Project"
df[which(df$network == "Jamaica"), "network"] <- "Windsor Cave"
df[which(df$network == "Texas"), "network"] <- "Big Bend"
...
df$metric <- gsub("interaction strength asymmetry", "interaction strength\nasymmetry", df$metric)
df$metric <- gsub("functional complementarity", "functional\ncomplementarity", df$metric)
...
for_multiplot <- c('nestedness', 'robustness_higher', 'NODF', 'modularity', "H2'",
                   'functional\ncomplementarity_higher')
...
# Make a matrix for the layout of the grobs
lay <- rbind(
  c(1, 2),
  c(3, 4),
  c(5, 6),
...
jpeg("Figures/grid_plot.jpg", units = "in", width = 7, height = 7, res = 300)
grid.arrange(
  grobs = lst,
  layout_matrix = lay,
  top = "7 networks",
...
jpeg("Figures/Guanacaste_grid_plot.jpg", units = "in", width = 7, height = 7, res = 300)
grid.arrange(
  grobs = hernani_lst,
  layout_matrix = lay,
  top = "2 networks",
